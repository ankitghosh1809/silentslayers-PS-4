        ...
        <div className="flex gap-6 min-h-[calc(100vh-2rem)] bg-slate-950 p-4">
            ...
            <div className="flex-1 flex gap-6">
                ...
                <div className="flex-1 glass rounded-[3rem] border-white/5 flex flex-col overflow-hidden">
                    <div className="p-8 border-b border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-indigo-500/20 flex items-center justify-center relative">
                                <div className="absolute inset-0 bg-indigo-500/20 rounded-2xl animate-ping opacity-20" />
                                <Bot className="w-7 h-7 text-indigo-400" />
                            </div>
                            <div>
                                <h2 className="text-xl font-black text-white uppercase italic tracking-tighter">AI Specialist</h2>
                    ...
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
                            ...
                            {messages.map((msg, i) => (
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        <div className={cn(
                                            "p-6 rounded-[2rem] text-sm font-medium italic leading-relaxed shadow-2xl",
                                            msg.role === 'user'
                        ...
                        {isTyping && (
                            <div className="flex gap-4">
                                <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center border border-white/10 shadow-lg">
                                    <Bot className="w-5 h-5 text-indigo-400" />
                                </div>
                                <div className="glass bg-white/5 px-6 py-4 rounded-3xl rounded-tl-none flex gap-1 items-center border-white/5 shadow-2xl">
                    ...
                    <div className="p-10 border-t border-white/5 bg-slate-900/40">
                        <div className="flex flex-wrap gap-2 mb-6">
                            {suggestions.map((s, i) => (
                                <button
                                    key={i}
                                    onClick={() => setInput(s)}
                                    className="px-4 py-2 glass bg-white/5 hover:bg-white/10 text-[10px] font-black text-indigo-400 uppercase tracking-widest rounded-xl transition-all border-white/5"
                                >
                                    {s}
                                </button>
                            ))}
                        ...
                        <form onSubmit={handleSend} className="relative group">
                            <input
                                ...
                                className="w-full bg-slate-900 border border-white/10 rounded-[2rem] py-5 pl-8 pr-20 text-white outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500/30 transition-all text-sm font-medium italic shadow-inner"
                                placeholder="Ask ReviewFlow AI..."
                            />
                            <button
                                type="submit"
                                className="absolute right-3 top-1/2 -translate-y-1/2 bg-indigo-600 hover:bg-indigo-500 p-3 rounded-2xl text-white transition-all shadow-lg shadow-indigo-500/20 active:scale-95 group-hover:rotate-12"
                            >
                ...
                <div className="w-80 flex flex-col gap-6">
                    <div className="glass p-8 rounded-[3rem] border-white/5 flex-1 relative overflow-hidden">
                    ...
                    <div className="glass p-8 rounded-[3rem] border-white/5 bg-gradient-to-b from-indigo-500/10 to-transparent">
